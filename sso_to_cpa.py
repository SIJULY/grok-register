...
def _prepare_ruyipage_path():
    """按环境变量加载 ruyipage，避免硬编码 D:/ruyipage。"""
...
def _handle_consent(sess, location, sso_token, code_verifier, email=""):
    ...
    opts = FirefoxOptions()
    firefox_binary = os.getenv("FIREFOX_BINARY") or _default_firefox_binary()
    opts.set_browser_path(os.path.expanduser(firefox_binary))
    opts.headless(True)
