...
GROK2API_DB = os.path.abspath(os.path.expanduser(
    os.getenv("GROK2API_DB") or os.path.join(os.path.expanduser("~"), "grok2api", "data", "backend.db")
))
...
def _assign_web_egress(node_id=1):
    ...
    if not os.path.exists(db):
        print(f"  [G2A-Web] 数据库不存在，跳过 egress 分配: {db}")
        return 0
    try:
